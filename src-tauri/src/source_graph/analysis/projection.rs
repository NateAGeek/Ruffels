...
use super::{
    classification::variable_name,
    orchestrator::IncludedNode,
    text::{compact_source_label, source_slice, source_span},
};
use crate::source_graph::model::{SourceNode, SourceNodeCategory, SourceParameter};
...
pub(super) fn graph_node_id(file_index: usize, kind: AstKind<'_>) -> String {
    let span = kind.span();
    format!("{:?}:{file_index}:{}:{}", kind.ty(), span.start, span.end)
}
...
fn node_label(kind: AstKind<'_>, source_text: &str, source_path: &str) -> String {
    match kind {
        ...
        _ => compact_source_label(source_text, kind.span(), &format!("{:?}", kind.ty())),
    }
}
